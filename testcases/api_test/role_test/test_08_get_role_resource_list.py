...
class TestGetResourceList():
    @allure.story("用例--获取角色的资源信息")
    @allure.description("该用例是针对获取角色资源信息的用户接口的测试")
    @pytest.mark.single
    @pytest.mark.parametrize("token,roleId,expect_result,expect_code, expect_msg",
                             api_data["test_get_resource_all"])
    def test_get_role_resource_list(self,get_admin_token_fixture,get_nopri_user_fixture,token,roleId,expect_result,expect_code, expect_msg):
        logger.info("*************** 开始执行用例 ***************")
        if token == "管理员token":
            token = get_admin_token_fixture
        elif token == "无权限token":
            token = get_nopri_user_fixture
        result = get_resource_list(token,roleId)
